...
from src.domain.events import BusinessEventType, CashReceipt, PartnerType, VendorPayment
    ...
    def register_customer_receipt(self, command: CustomerReceiptCommand) -> ServiceResult:
        try:
            event = CashReceipt(
                ...
                reference=self._workflow.resolve_reference(BusinessEventType.CASH_RECEIPT, command.reference),
    ...
    def register_vendor_payment(self, command: VendorPaymentCommand) -> ServiceResult:
        try:
            event = VendorPayment(
                ...
                amount=command.amount,
                reference=self._workflow.resolve_reference(BusinessEventType.CASH_PAYMENT, command.reference),
                description=command.description,
            )
        except DomainError:
            logger.warning(
