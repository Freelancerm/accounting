...
from src.services.commands import CustomerReceiptCommand, VendorPaymentCommand
...
class CashService:
    ...
    def register_customer_receipt(self, command: CustomerReceiptCommand):
        try:
            event = CashReceipt(
    ...
    def register_vendor_payment(self, command: VendorPaymentCommand):
        try:
            event = VendorPayment(
                entry_date=command.entry_date,
                partner=self._workflow.build_partner(
