...
class SQLiteRepository:
    ...
    def search_partners(self, query: str, partner_type: PartnerType, *, limit: int = 5) -> list[Partner]:
        return self.partners.search_by_name(query, partner_type, limit=limit)

    def find_partner_by_name(self, name: str, partner_type: PartnerType) -> Partner | None:
        return self.partners.find_by_exact_name(name, partner_type)
    ...
    def next_reference(self, event_type: BusinessEventType) -> str:
        return self.documents.next_reference(event_type)
